...
export async function GET(req: NextRequest) {
  ...
  if (!q || q.length < 2) return NextResponse.json({ products: [], maxPage: 1, total: 0 })

  try {
    const fp = path.join(process.cwd(), 'public', 'products.json')
    if (!fs.existsSync(fp)) return NextResponse.json({ products: [], maxPage: 1, total: 0 })
    const all: Product[] = JSON.parse(fs.readFileSync(fp, 'utf-8'))
